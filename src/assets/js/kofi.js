const kofiWidgetOverlayConfig = {
  ...
  "floating-chat.core.position.bottom-left":
    "position: relative; bottom: 50px; left: 10px; width: 160px; height: 65px;",
  "floating-chat.cssId": "",
...
var kofiWidgetOverlayFloatingChatBuilder =
  ...
  function (config, _utils) {
    ...
    function getMobiContainerFrameId() {
      return (
        "kofi-wo-container-mobi" + _configManager.getValue(_myType, "cssId")
      );
    }
    ...
    function createButtonContainerIframe(iframeId, mainStyleSheetFile) {
      ...
      var _timer = setInterval(function () {
        ...
        if (doc && doc.readyState == "complete") {
          ...
          var parentWrapper = document.getElementsByClassName(
            _topContainerWrapClass
          )[0];
          var mobiParentWrapper = document.getElementsByClassName(
    ...
    function attachDonateButton(
      iframeContainerElement,
      iframeId,
      selectors,
      heightLimits
    ) {
      const donateButton = iframeContainerElement.getElementById(
        `${getButtonId()}`
      );
    ...
    var write = function (parentElementId) {
      ...
      var iframeContainerElement = createButtonContainerIframe(
        iframeId,
        "assets/css/kofi-chat.css"
      );
      var mobiIframeContainerElement = createButtonContainerIframe(
        mobiIframeId,
        "assets/css/kofi-chat.css"
      );
      _utils.loadStyleSheet("assets/css/kofi.css", document);
    ...
    function activateKofiIframe(iframeId, selectors, heightLimits) {
      ...
      const kofiIframeState = donateButton.classList.contains("closed")
        ? "open"
        : "close";
      toggleKofiIframe(
        iframeId,
    ...
    function slidePopupOpen(popup, finalHeight) {
      popup.style = `z-index:10000;height: ${finalHeight}px!important; transition: height 0.5s ease, opacity 0.3s linear; opacity:1;`;
    ...
    function insertPopupHtmlIntoBody(donateButton, selectors, parentElementId) {
      ...
      closerContent.innerHTML = _configManager.getValue(
        _myType,
        "closer",
        true
      );
      ...
      popupIFrameContainer.id =
        popupId + selectors.popupIframeContainerIdSuffix;
      popup.appendChild(popupIFrameContainer);
    }
    function toggleKofiIframe(
      iframeId,
      state,
      donateButton,
      selectors,
      ...
      if (state === "open") {
        var iframeContainerParent =
          document.getElementById(iframeId).parentElement;
        var finalHeight = window.innerHeight - 75;
        ...
        var widgetPageLoadStateIndex = widgetPageLoadInitiatedStates.findIndex(
          function (s) {
            return s[0] == donateButton;
          }
        );
        var widgetPageLoadInitiated =
          widgetPageLoadInitiatedStates[widgetPageLoadStateIndex][1];
        if (!widgetPageLoadInitiated) {
          var popupIFrameContainerId =
            popupId + selectors.popupIframeContainerIdSuffix;
          ...
          widgetPageLoadInitiatedStates[widgetPageLoadStateIndex] = [
            donateButton,
            true,
          ];
        }
    ...
    var getHtml = function () {
      var donateButtonImage = _configManager.getValue(
        _myType,
        "donatebutton.image"
      );
      ...
      var donateButtonCTAText = _configManager.getValue(
        _myType,
        "donateButton.text"
      );
      var donateButtonTextColor = _configManager.getValue(
        _myType,
        "donateButton.text-color"
      );
      var body =
        "<style> body {right: 5px; top: 5px} .hiddenUntilReady { display: none; } </style>" +
        ...
        `<span class="kofi-text" style="margin-left: 8px; color:${donateButtonTextColor}">${donateButtonCTAText}</span>`;
      ("</div>");
      return body;
    };
...
var kofiWidgetOverlayUtilities =
  ...
  function () {
    ...
    const loadKofiIframe = function (pageId, parentElementId, iframeStyle) {
      ...
      const tryLoad = function () {
        if (!_iframeLoading) {
          _iframeLoading = true;
          let url =
            kofiWidgetOverlayConstants.kofiRoot +
            pageId +
    ...
    const loadStyleSheet = function (styleSheetHref, targetDocument) {
      ...
      var styleSheet = targetDocument.querySelectorAll(
        '[href="' + styleSheetHref + '"]'
      );
...
var kofiWidgetOverlay =
  ...
  (function () {
    ...
    var _buildStrategy = {
      "floating-chat": {
        ...
        write: function (parentId, config, utils) {
          return new kofiWidgetOverlayFloatingChatBuilder(config, utils).write(
            parentId
          );
        },
        getBody: function (config, utils) {
          return new kofiWidgetOverlayFloatingChatBuilder(
            config,
            utils
          ).getHtml();
        },
    ...
    function getBuilder(widgetType) {
      var buildStrategy =
        _buildStrategy[widgetType] === undefined ? "empty" : widgetType;
    ...
    const setConfigDefaults = function (config, widgetType, pId, instanceId) {
      ...
      config[widgetType + ".cssId"] =
        config[widgetType + ".cssId"] !== undefined &&
        config[widgetType + ".cssId"] !== ""
          ? config[widgetType + ".cssId"]
          : instanceId;
